...
void overlayFitMuon()
{
  ...
  TGraphErrors *g1 = new TGraphErrors(nbin,x_b,y_b_barrel_data,ex,ey_barrel_data);
  //decorate the graph
  ...
  //Declare the graph
  //TGraphErrors *g1 = new TGraphErrors(nbin,x_b,y_b_endcap,ex,ey_endcap);
  TGraphErrors *g1 = new TGraphErrors(nbin,x_b,y_b_endcap_data,ex,ey_endcap_data);
  //decorate the graph
  decorate(g1,";Probe p_{T} [GeV]; Efficiency",0.5,1.1,kRed,21,kRed,2);
  g1->SetTitle("TriggerEfficiency_Endcap");
  auto legend = new TLegend(0.85,0.60,0.55,0.70);
  legend->AddEntry(g1,"Data","lep");
  legend->SetFillStyle(2);
  g1->Draw("ap");
